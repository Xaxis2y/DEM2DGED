...
steep-terrain conversion accuracy. `-resample optimize` always benchmarked
candidates at a fixed 2x hold-out ratio regardless of the actual requested
decimation ratio, so on steep terrain at 8x-16x it could recommend Cubic when
Nearest Neighbor was 1.5-3x more accurate. The v0.49 Gaussian anti-alias
`-prefilter` measurably worsens point accuracy at every ratio/resampler
combination tested on steep terrain. The `rms` resampler is mathematically
unsound for elevation data -- it can be off by 100+ m and even return the
wrong sign. All three are now fixed: `-resample optimize` benchmarks at the
actual requested ratio via a new `pick_holdout_factor()` and scores `average`
as a fifth candidate; the `-prefilter` help/GUI text now warns that it
worsens point accuracy on steep terrain; and `rms` has been removed outright
(it now raises `SystemExit` pointing to `average` instead). Covered by 21 new
regression tests in `tests/test_v057_regressions.py`. v0.57.1 itself is a
documentation/polish pass over v0.57.0 with no further logic changes. See
[`REQUIREMENTS_COMPLIANCE_V0.57.1.md`](REQUIREMENTS_COMPLIANCE_V0.57.1.md).
...
| Level | Approx GSD | Predicted H. accuracy (CE90) | Predicted V. accuracy (LE90) | Tile size (GEO) | Tile size (UTM) |
|-------|-----------|-------------------------------|-------------------------------|-----------------|-----------------|
| 0     | ~1000 m   | 50 m    | 30 m     | 1°              | —               |
| 1     | ~100 m    | 50 m    | 30 m     | 1°              | —               |
| 2     | ~30 m     | 23 m    | 18 m     | 1°              | —               |
| 3     | ~12 m     | 15 m    | 12.4 m   | 1°              | —               |
| 4b    | ~5 m      | 6 m     | 5 m      | 15'             | 25 km × 25 km   |
| 4     | ~4 m      | 5 m     | 4 m      | 15'             | 25 km × 25 km   |
| **5** | **~2 m**  | **3 m** | **2 m**  | **6'**          | **10 km × 10 km** |
| 6     | ~1 m      | 2 m     | 1 m      | 3'              | 5 km × 5 km     |
| 7     | ~0.5 m    | 1 m     | 0.5 m    | 1.5'            | 2.5 km × 2.5 km |
| 8     | ~0.25 m   | 0.5 m   | 0.25 m   | 1'              | 2.5 km × 2.5 km |
| 9     | ~0.125 m  | 0.25 m  | 0.12 m   | 1'              | 1.25 km × 1.25 km |
...
A level's post spacing (the "Approx GSD" column) is the *distance between* the fixed grid points that actually get a measured elevation value — everything between those points is filled in by resampling, not measured directly. On flat or gently rolling terrain that's a safe approximation: neighbouring posts differ by very little, so any resampler reconstructs the surface between them closely. On steep terrain — ridgelines, canyon walls, cliff edges, terraced slopes — elevation can change by tens of metres over a horizontal distance smaller than a single post spacing, so the values between posts are effectively invented rather than interpolated, and how far that invented value lands from the truth depends on both the level chosen and the resampling method used to build the tile.
