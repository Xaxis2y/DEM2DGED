...
def create_version_file(target_dir):
    ...
    print("[OK] Refreshed VERSION.txt header (changelog preserved: "
...
# v0.56: three more prefixes. The review that produced this release left
# _backup_v0.55.0/ (a full pre-patch snapshot), diagnostics/ (harness logs
# and synthetic scratch rasters) and release_gate/work/ (real conversion
# output, easily hundreds of MB) sitting in the project folder. None is a
# release input, and all three would otherwise have been zipped -- the same
# shape of mistake the .tmp/.log/.bak suffixes were added for in v0.42.
EXCLUDE_DIR_PREFIXES = ("dem2dged_validate_v",   # unzipped staging snapshots
                        ...
                        "_backup",               # pre-patch source snapshots
                        "diagnostics",           # DIAG harness logs + scratch
                        "release_gate")          # release gate logs + work dir
...
# v0.56: two superseded release artefacts. Both were replaced by
# v0.56.0-named files and are kept on disk only until the release is
# confirmed good; neither belongs in the archive.
EXCLUDE_FILES = {"DIAG_dem2dged_v0.55.0.py",
                 "REQUIREMENTS_COMPLIANCE_V0.55.0.md",
                 "DEM2DGED_User_Manual_old.docx", "note_and_issue.md",
