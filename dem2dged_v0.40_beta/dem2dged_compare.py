"""
dem2dged_compare.py  -  Resampling-method accuracy comparison for dem2dged.
...
New in v0.33. Compares DGED tile sets produced with different gdalwarp
resampling algorithms (Nearest Neighbor / Bilinear Interpolation / Cubic
Convolution) against the ORIGINAL source DEM and writes an HTML
"Resampling Comparison Report" ranking the methods by accuracy.

Two complementary measurements are made per method:

1. HOLD-OUT CROSS-VALIDATION (primary ranking metric).
   Every other source post (a 2x-decimated "training" grid) is resampled
   with the method's algorithm back onto the FULL source grid, and the
   reconstructed values at the WITHHELD posts -- real, measured elevations
   the algorithm never saw -- are compared against their true values.
   This directly measures how accurately the algorithm reconstructs real
   terrain between known posts, which is exactly the job it performs when
   producing DGED posts that fall between source posts. It cannot be
   gamed: a nearest-neighbor round trip scores a perfect (but
   meaningless) zero on test 2 below, while genuinely mispredicting the
   terrain between posts -- this test exposes that.

2. ROUND-TRIP RESIDUAL (secondary, run on the ACTUAL delivered tiles).
   All tiles of the method folder are mosaicked (in-memory VRT), warped
   BACK onto the source raster's own grid (bilinear back-warp for EVERY
   method, so the back-transform cost is identical and the comparison
   fair), and differenced against the original source post-by-post.
   This checks the delivered product end-to-end: tiling, nodata, data
   type, and how much of the original signal survives. It also yields the
   output min/max vs. source min/max and the overshoot beyond the source
   range (a fingerprint of cubic "ringing" at sharp terrain breaks).

The method with the LOWEST hold-out RMSE is ranked most accurate
(ties broken by the round-trip RMSE).

Public API:
    COMPARISON_METHODS                     - the three test methods
    compute_method_stats(src, folder, alg) - metrics dict for one method
    write_comparison_report(entries, path) - render the HTML report
    pick_best_resampling(src, angular)     - new in v0.36; auto-select the
                                              most accurate method for one
                                              source WITHOUT writing tiles
                                              or a report (see its own
                                              docstring)
"""
...
try:
    from dem2dged_lib import VERSION
except Exception:
    VERSION = "0.40"   # fallback; keep in sync with dem2dged_lib.VERSION
