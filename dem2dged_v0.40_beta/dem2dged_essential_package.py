...
# -*- coding: utf-8 -*-
"""
DEM2DGED Essential Toolkit Packaging Script - v1.0.0

SPDX-License-Identifier: GPL-2.0-or-later
Copyright (c) 2026 Eui Soo SON

Builds a single, LEAN zip containing just the three things needed to run
the toolchain -- no DEM data, no test fixtures, no PyInstaller build output:

    dem2dged/            the core CLI/GUI/converter/validator (source + docs)
    arcgis_qa_toolbox/   the ArcGIS Pro QA comparison toolbox (companion, v1.0.2)
    DGED Loader/         the ArcGIS Pro tile-loading toolbox (companion)

Deliberately excluded (this is what makes it "lean"):
    tests/               815 MB -- almost entirely tests/DGIWG Test Data/
                         (real DEM/raster test fixtures + generated test
                         output rasters), not needed to USE the tools
    build/               472 MB -- PyInstaller intermediate build cache,
                         100% regenerable from source via rebuild_exe.bat
    dist/                463 MB -- compiled dem2dged.exe / dem2dged_validate.exe,
                         also regenerable from source via rebuild_*.bat
    *.zip                any zip already sitting in the project folder
                         (this script's own prior output, the QA toolbox's
                         v1.0.0/1.0.1/1.0.2 zips, etc.) -- never nest an old
                         package inside a new one
    __pycache__, *.pyt.xml   Python bytecode cache and ArcGIS Pro's own
                         auto-generated toolbox metadata sidecars -- both
                         regenerate themselves the moment the tools are used

This mirrors dem2dged_package.py's own EXCLUDE_DIRS convention (it already
excludes build/, dist/, and "DGED Loader" from ITS release zip -- the first
two because they're build output, the third because DGED Loader has its
own separate release process) -- this script just applies the same "source
and docs only" philosophy across all three components at once, into one
combined bundle instead of three separate ones.

Usage:
    python dem2dged_essential_package.py

Run from any Python 3 interpreter -- arcpy/GDAL are not required, this
script only touches plain files and the zip.
"""
