"""
dem2dged_gui.py  –  Simple GUI for DEM → DGED conversion.
...
Uses the GDAL Python API directly (no subprocess calls to gdalwarp /
gdal_edit.py) so PyInstaller can produce a fully self-contained .exe.

v0.33: resampling is now user-selectable. A "Resampling Method" dropdown
(default: Auto — the validator-safe automatic choice that has been used
since v0.20) lets the operator force Nearest Neighbor, Bilinear
Interpolation, or Cubic Convolution; the choice is passed through
dl.pick_resampler(), the same override path the CLI's -resample flag has
used since v0.20. A new "Resampling Comparison Test" section adds one
checkbox per method: any checked subset (1, 2, or all 3) is converted
side-by-side into test folders test_1_nearest_neighbor /
test_2_bilinear_interpolation / test_3_cubic_convolution, and a
DGED_Resampling_Comparison_Report.html (see dem2dged_compare.py) ranks the
methods by round-trip RMSE against the original source DEM.

v0.36: two additions working together. (1) A "Skip elevation sanity check"
checkbox and a pre-flight check (dl.sanity_check_elevation_source()) run
before every file: if the source's value range and filename look like a
non-elevation raster (aspect/direction/curvature -- DGED is elevation-only),
the file is logged and skipped rather than silently producing a bad
delivery, unless the checkbox is on. (2) The Resampling Method dropdown
gains an "Optimize" entry: instead of Auto's fixed GSD-ratio rule of thumb,
it measures Nearest/Bilinear/Cubic against the source DEM itself (same
hold-out cross-validation as the Comparison Test, but no tiles or report
written) and uses whichever is most accurate for that file -- except when
(1) flags the source as angular/circular data, in which case RMSE would be
meaningless across the 0/360 wraparound and Nearest Neighbor is used
directly. See dl.resolve_resampler() / dem2dged_compare.pick_best_resampling().

v0.28: the tile-extent, data-type, naming, and metadata-sidecar logic
below now calls straight into dem2dged_lib.py -- the same functions
dem2dged_geo.py / dem2dged_utm.py use -- instead of a second, independently
maintained copy. The second copy is what let this file's output quietly
fall out of spec (half-post pixel misregistration, wrong data type below
level 3, stale filenames, incomplete metadata) while the CLI converters
were fixed out from under it in v0.27; sharing the implementation removes
that whole class of drift going forward. Only the actual gdal.Warp() call
stays local, since the GUI needs the Python API (no external gdalwarp
binary) to work from inside a frozen .exe.
...
try:
    ...
    APP_VERSION = "0.40"   # fallback; keep in sync with dem2dged_lib.VERSION
    APP_VERSION_DISPLAY = "0.40-beta"
                        ...
                        if meth:
                            ...
                            if do_validate:
                                try:
                                    ...
                                    rep, tiles = dv.run_validation(
                                        out_dir, src=src, resample=meth[1])
                            ...
                            if do_validate and not self._stop.is_set():
                                ...
                                try:
                                    ...
                                    # Finding 2).
                                    rep, tiles = dv.run_validation(
                                        out_dir, src=src, resample=resamp_used)
