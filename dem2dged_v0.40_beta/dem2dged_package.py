#!/usr/bin/env python3
"""
DEM2DGED Packaging Automation Script - v0.40-beta
...
VERSION = "0.40"
# v0.39: beta release. VERSION stays a bare MAJOR.MINOR (the value the whole
# tool and audit_pure.py key off); the beta qualifier rides in RELEASE_STAGE
# so the zip/name reads "dem2dged_v0.39_beta" and VERSION.txt shows
# "0.39-beta", without changing the audited numeric version.
RELEASE_STAGE = "beta"
...
- Verification re-cut of v0.39-beta. A second review pass over the extracted
  ...
  aware predictor). No functional change from v0.39-beta -- the five
  v0.39-beta fixes are carried forward and re-verified. QUICKSTART.html and
  the user manual refreshed to v0.40-beta.
...
# for testing -- can be gigabytes), "tests" and "tests_beta" (the local test
# harness + its generated tiles/reports/logs), and any "output*" conversion
# folders are all excluded: none are part of the source/docs release, and the
# DEM data in particular must never be bundled into the shipped zip.
EXCLUDE_DIRS = {"build", "dist", "__pycache__", ".pytest_cache", "_v027_sync",
                 "DGED Loader", "ArcGIS_PRO_QA_toolbox", "DEM", "tests",
                 "tests_beta", "_verify_pages"}
