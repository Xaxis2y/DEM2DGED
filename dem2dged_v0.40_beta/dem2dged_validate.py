...
Version: 0.40-beta (shared tool version, imported from dem2dged_lib.VERSION)

Changelog
    0.40-beta  Verification re-cut of v0.39-beta; no validator logic change.
          Re-audited and re-run end to end (19/19); the v0.39-beta Section H
          geoid fix and the source-type WARN are carried forward.
    0.39-beta  First public beta (project-wide review pass).
...
import argparse, os, re, sys, math, glob
import xml.etree.ElementTree as ET

# ── Fix GDAL / PROJ data paths when running as a frozen PyInstaller exe ───────
# Without this, dem2dged_validate.exe fails (or silently misbehaves) as soon
# as it touches GDAL/PROJ, because the bundled gcs.csv / proj.db files are
# extracted under sys._MEIPASS at runtime but GDAL/PROJ have no way to know
# that unless GDAL_DATA / PROJ_LIB / PROJ_DATA point there explicitly. This
# mirrors the same fix already used in dem2dged_gui.py.
if getattr(sys, "frozen", False):
    _base = sys._MEIPASS
    os.environ.setdefault("GDAL_DATA", os.path.join(_base, "gdal"))
    os.environ.setdefault("PROJ_LIB",  os.path.join(_base, "proj"))
    os.environ.setdefault("PROJ_DATA", os.path.join(_base, "proj"))
...
try:
    ...
    sys.exit(
        "ERROR: could not import GDAL/numpy (%s).\n\n"
        "dem2dged_validate.py needs the same GDAL-enabled Python environment\n"
        "used to run dem2dged.py -- activate it first, e.g.:\n\n"
        "    conda activate DGED\n"
        "    python dem2dged_validate.py <tile_folder> ...\n\n"
        "If that environment doesn't exist yet, create it with install.bat\n"
        "(Windows) or install.sh (Linux/macOS), or manually:\n\n"
        "    conda create --name DGED --channel conda-forge gdal python=3.10 -y"
        % e)

try:
    from dem2dged_lib import (VERSION, VERSION_DISPLAY, PL,
                              level_tilesize_and_spatial_resolution,
                              zone_lon_spacing, output_type_for_level,
                              TOC_FILENAME, utm_name_field_widths,
                              compute_tile_stats, OVERSHOOT_PRONE_RESAMPLERS,
                              describe_source_type)
except ImportError as e:
    # No fallback copy of these tables on purpose: dem2dged_lib.py is the
    # single source of truth for every DGED table this validator checks
    # against (see its changelog). A hand-copied duplicate here would only
    # be exercised on this error path, where nothing would ever notice it
    # silently drifting out of sync -- which is exactly what happened before
    # (the old fallback's GEO level 8-9 tile size had gone stale). Failing
    # loudly is safer than validating against numbers nobody is maintaining.
    sys.exit(
        "ERROR: could not import dem2dged_lib (%s).\n\n"
        "dem2dged_validate.py must be run from the same folder as "
        "dem2dged_lib.py (or with it on PYTHONPATH) -- it supplies the DGED "
        "tables every check below relies on." % e)

gdal.UseExceptions()

NODATA = -32767.0
ELEV_MIN_SANE, ELEV_MAX_SANE = -500.0, 9100.0   # Dead Sea .. Everest + margin

# Levels 0-3 (GEO only) omit the "Gt<letter>" tile-size segment in the
# current spec-form name (dem2dged_lib.geo_tile_basename, v0.27+), so that
# segment is optional here -- a pre-v0.27 file that still carries it also
# matches. An optional 2-4 letter producer-organisation code (-org) can sit
# right before the coordinate in any level. UTM tiles never omit the letter:
# UTM only exists for levels 4b and up (dem2dged_utm.resolve_level_utm).
GEO_RE = re.compile(r"^DGEDL(?P<lv>4b|\d)"
                    r"(?:Gt(?P<letter>[A-Z]))?_"
                    r"(?:(?P<org>[A-Z]{2,4})_)?"
                    r"(?P<lat>\d{2,6})(?P<hemi>[NS])"
                    r"(?P<lon>\d{3,7})(?P<east>[EW])_"
                    r"(?P<src>[A-Z])_(?P<sec>[TSCRU])_(?P<ver>\d{2})$")
# The northing/easting subfields are deliberately matched permissively
# (\d{1,7} / \d{1,6}) rather than at their exact spec widths. Spec 12.1
# requires fixed-width zero-padded fields, and check B below enforces that
# via dem2dged_lib.utm_name_field_widths() -- but a pre-v0.34 tile carrying
# a short field should get that precise diagnostic, not a blanket "filename
# does not match DGED naming convention" that says nothing about what is
# actually wrong with it. (v0.34)
UTM_RE = re.compile(r"^DGEDL(?P<lv>4b|\d)Ut(?P<letter>[A-Z])_"
                    r"(?:(?P<org>[A-Z]{2,4})_)?"
                    r"(?P<zone>\d{1,2})(?P<hemi>[NS])(?P<northing>\d{1,7})_"
                    r"(?P<easting>\d{1,6})_"
                    r"(?P<src>[A-Z])_(?P<sec>[TSCRU])_(?P<ver>\d{2})$")
...
def overall_result(n_pass: int, n_warn: int, n_fail: int) -> str:
...
def _compare_edge(a_path, a_win, b_path, b_win, a_name, b_name, kind, rep):
    ...
    """
    try:
        a = gdal.Open(a_path); b = gdal.Open(b_path)
...
def check_edges(tiles, rep, max_pairs=20):
    ...
    for t in tiles:
        ...
        if e:
            try:
                ds = gdal.Open(e["path"]); ey = ds.RasterYSize; ds = None
        ...
        if nb:
            try:
                ds = gdal.Open(nb["path"]); nx, ny = ds.RasterXSize, ds.RasterYSize; ds = None
