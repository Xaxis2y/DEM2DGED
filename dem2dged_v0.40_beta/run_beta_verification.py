...
# -*- coding: utf-8 -*-
"""
run_beta_verification.py  -  End-to-end release verification driver for the
dem2dged v0.40-beta candidate.

SPDX-License-Identifier: GPL-2.0-or-later
Copyright (c) 2026 Eui Soo SON

Run from the dem2dged/ project folder inside the GDAL-enabled conda
environment ("conda activate DGED") the tool itself uses:

    python run_beta_verification.py

All output goes under tests_beta/ (which the packager excludes). Exit code
0 = every gated step passed, 1 = at least one gated step failed.

How results are judged
----------------------
The validator's checks fall into two groups, and this harness treats them
differently on purpose:

  * STRUCTURAL (Sections A-G): file pairing, DGED naming, tile-size letter,
    UTM field widths, data type per level, NoData, AREA_OR_POINT, LZW, CRS/
    EGM2008, pixel size, origin alignment, dimensions, XML, elevation-range
    sanity, and adjacent-tile edge overlap. These are the hard spec-
    compliance checks. Every conversion below is GATED on zero structural
    FAILs.

  * SOURCE COMPARISON (Section H/H2): min/max/mean and sample-window pixel
    diffs of the delivered tiles against the source DEM. This is an accuracy
    HEURISTIC with a metres-level tolerance. It is expected to exceed
    tolerance for Nearest Neighbor / Cubic on steeply-varying downsampled
    terrain, and for partial/coastal deliveries -- so it is only GATED on the
    one run that uses an accurate resampler at near-native resolution
    (`gate_source=True`); for the others its result is reported for
    information but does not fail the suite.
"""
...
OUT_DIR = os.path.join(HERE, "tests_beta")
...
try:
    ...
    summ("FATAL: could not import GDAL/numpy (%s)." % e)
...
def subset(src, dst, span_x, span_y, off_x=0.0, off_y=0.0):
    ...
    ds = gdal.Translate(dst, src, projWin=[x0, y1, x1, y0])
    ds = None
    return dst
...
def step_environment():
    ...
    lines = ["dem2dged v0.40-beta verification  -  %s" % _stamp(),
...
def main():
    summ("=" * 74)
    summ(" dem2dged v0.40-beta  -  release verification")
    summ(" %s" % _stamp())
    summ("=" * 74)
