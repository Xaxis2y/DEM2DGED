"""
selftest_resampling_comparison.py  -  End-to-end self-test for the v0.33
resampling comparison feature, using the REAL GDAL installation.

SPDX-License-Identifier: GPL-2.0-or-later
Copyright (c) 2026 Eui Soo SON

Version: 0.34 (shared tool version, see dem2dged_lib.VERSION)

Run from the project folder in an environment where GDAL is installed
(the same conda env used by rebuild_exe.bat):

    python selftest_resampling_comparison.py

What it does:
  1. Creates a synthetic 100x100 EPSG:4326 source DEM (smooth terrain +
     a Gaussian hill + a moderate ridge) in a temporary folder.
  2. Converts it with all three comparison methods (Nearest Neighbor /
     Bilinear Interpolation / Cubic Convolution) through the SAME
     convert_geo() code path the GUI uses, into the test folders
     test_1_nearest_neighbor / test_2_bilinear_interpolation /
     test_3_cubic_convolution.
  3. Runs dem2dged_compare.compute_method_stats() on each folder and
     writes DGED_Resampling_Comparison_Report.html.
  4. Checks the expected physics:
       - hold-out RMSE of Nearest Neighbor is the worst of the three
         (it cannot reconstruct terrain between posts),
       - round-trip RMSE of Nearest Neighbor is ~0 when upsampling
         (it copies source posts exactly),
       - the report file exists and names a "Most Accurate" method.
