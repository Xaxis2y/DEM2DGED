...
def create_environment():
    ...
    # v0.42: this used to pass pip a "-y" flag. pip has no such option (only
    # conda does) -- confirmed by running it directly: pip prints its usage
    # banner and exits non-zero without installing anything. run_command()
    # only warns on a non-zero return, so this step has always silently
    # no-op'ed; the script still printed "Environment Setup Complete!"
    # afterwards. Dropped the flag so the install actually runs. In practice
    # this mattered less than it looks: matplotlib/pillow/scipy are not
    # imported anywhere in the dem2dged source, and numpy already arrives
    # transitively as a dependency of conda-forge's gdal package (Step 2).
    if not run_command(
        f'conda run -n {ENVIRONMENT_NAME} pip install --break-system-packages numpy matplotlib pillow scipy',
...
def verify_environment():
    ...
    if result.returncode == 0:
        print(f"✓ Environment '{ENVIRONMENT_NAME}' exists.")
        print("\nInstalled packages:")
        print(result.stdout)
        return True
    else:
