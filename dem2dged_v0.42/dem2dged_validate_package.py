...
Changes in v0.42:
- Packaging fix, no validator logic change: this script's file list never
  included LICENSE, so the validator-only bundle it produces
  (dem2dged_validate_v{VERSION}/, and the zip built from it) shipped
  without one despite MANIFEST.md always having said it does -- a real
  compliance gap for a GPL-2.0-or-later project, not a cosmetic one.
  LICENSE is now copied in like the other five files.
- Also part of this release-readiness pass (see VERSION.txt / dem2dged_
  package.py for the full write-up, none of it touches this module):
  tests/ restored to the source release after being found silently
  excluded from it; a stray .tmp scratch file and now .log/.bak excluded
  from release zips; a broken "pip install -y" call fixed in the
  alternate environment-setup script; a malformed table in README.md
  fixed.

Changes in v0.41:
- REPAIR RELEASE -- the v0.40 dem2dged_validate.py did not byte-compile and
  could not be run, imported or built at all. A block was missing between
  the end of the module docstring and the body of overall_result(): the
  docstring's closing quotes, every import, the NODATA / ELEV_MIN_SANE /
  ELEV_MAX_SANE constants, _STATUS_ORDER, the GEO_RE and UTM_RE filename
  patterns, and the "def overall_result(...)" line. Python read the module
  docstring as continuing into overall_result()'s own docstring and stopped
  with "IndentationError: unexpected indent (line 247)". Anything that
  imports this module degraded silently: dem2dged.py's auto-validation
  logged "could not import dem2dged_validate" and wrote no report, and the
  GUI's "Validate after conversion" checkbox stayed disabled.
- The missing block is restored. GEO_RE / UTM_RE were rebuilt from spec 12.1
  and re-verified against dem2dged_lib.geo_tile_basename() /
  utm_tile_basename() output for every product level (0-3, 4b, 4-9), both
  hemispheres, all UTM zone forms, with and without the optional 3-letter
  organisation code, plus the pre-v0.34 short-northing legacy name (which
  must still PARSE so the field-width check can report a precise error),
  the pre-v0.27 GEO L0-3 "Gt<letter>" legacy form, negative cases, and
  GEO/UTM cross-matching. NODATA is -32767 (spec 13.1, matching the value
  the converters pass to gdalwarp -dstnodata) and the sane elevation band
  is [-500, 9000] m, the "far below the Dead Sea or above Everest" range
  the report's own explanatory text describes.
- GDAL exception contract: this module ran with gdal exceptions OFF from
  the CLI but ON from the GUI (which calls gdal.UseExceptions() and then
  imports this module), so identical input took two different error paths.
  dem2dged_lib.py now pins it for everyone and supplies gdal_open(), which
  returns None either way; check_tile(), _compare_edge(), check_edges() and
  check_source() all use it. Two consequences were real bugs: a corrupt
  tile, and an unreadable -src source DEM, each killed the ENTIRE
  validation run with an AttributeError instead of being reported. Both
  are now a clean FAIL for that one item.
- No intended behavioural change relative to the v0.39 validator: the hard
  spec checks (naming, grid geometry, data type, NoData, CRS/EGM2008 tag,
  XML, elevation statistics, edge overlap) and the Section H comparison are
  as they were. The v0.39 Section H geoid fix and the advisory source-type
  WARN are carried forward.
- (This entry was missing from this script's own embedded template --
  VALIDATOR_VERSION.txt on disk had it, but re-running this packaging
  script would have silently dropped it from a regenerated copy. Restored
  here so the two cannot drift apart again.)

Changes in v0.40:
- Verification re-cut of v0.39; no validator logic change. Re-audited
  and re-run end to end (19/19). The v0.39 Section H geoid fix (compare
...
def create_package_zip():
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        files_to_include = [
            ...
            # v0.42: LICENSE was missing from this list. MANIFEST.md has
            # always described the validator-only bundle as including it
            # ("validator + dem2dged_lib.py + manual + LICENSE + README +
            # rebuild script + VALIDATOR_VERSION.txt"), but nothing in this
            # script ever actually copied it in -- confirmed by the fact
            # that dem2dged_validate_v0.41/, the staging folder this script
            # produces, has no LICENSE file. For a GPL-2.0-or-later project
            # that's a real compliance gap, not a cosmetic one.
            "LICENSE",
        ]
