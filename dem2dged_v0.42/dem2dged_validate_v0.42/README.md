...
| Version | Change |
|---|---|
| v0.42 (2026-08-07) | **Release-readiness review of the v0.41 package — packaging and docs only, no functional change.** Reviewing the actual extracted `dem2dged_v0.41` folder (as opposed to re-reading `CODE_REVIEW_v0.41.md`'s claims about it) found it was not ready to ship. **`tests/` was missing from the release folder itself**, despite `MANIFEST.md` listing five files under it as required and instructing `pytest -q` "after unzipping" — `pytest` would fail immediately with "file or directory not found: tests". Root cause: `dem2dged_package.py`'s `EXCLUDE_DIRS` excluded `"tests"` (listed twice) from the source zip it builds, directly contradicting `MANIFEST.md`'s own definition of the release contents; `"tests"` is no longer excluded. **The validator-only bundle never included `LICENSE`** despite `MANIFEST.md` saying it does — `dem2dged_validate_package.py`'s file list simply never had it; added. **`lu49gpd00.tmp`**, a stray 311 KB scratch file (a PDF wearing a `.tmp` extension) sitting in the project root, was left unresolved as an open item in `CODE_REVIEW_v0.41.md`; `.tmp`/`.log`/`.bak` are now excluded from release zips. **`dem2dged_anaconda_environment.py`** called `pip install -y ...` — pip has no `-y` flag and rejects it outright (confirmed by running it directly), so that step silently installed nothing while the script still reported success; flag removed. **This table's own header row** was missing its separator and wasn't rendering as a table; fixed. Both packaging scripts' embedded `VERSION.txt`/`VALIDATOR_VERSION.txt` templates were re-run and re-verified with `audit_pure.py` (`RESULT: 0 problem(s)`) after every change. |
