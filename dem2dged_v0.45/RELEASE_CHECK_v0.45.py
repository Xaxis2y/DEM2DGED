...
r"""Release gate for dem2dged v0.45 -- run this locally, return the logs.
...
---------------------------------------------------------------------
    conda activate DGED
    cd /d C:\Users\xaxis\Documents\ES_Project\dem2dged_v0.41
...
Everything lands in _release_check_logs\ :
    ...
    04b_preflight.txt       the v0.45 guards, exercised through the real CLI
...
# v0.45: the two PyInstaller builds (step 12) take several minutes each.
...
def step_environment():
    ...
    try:
        ...
        # v0.45: this used to assert "You are almost certainly not in the
        ...
        try:
            ...
            lines.append("Use 'python RELEASE_CHECK_v0.45.py', not "
                         "'RELEASE_CHECK_v0.45.py'.")
    ...
    # v0.45: on Windows `conda` is a .bat shim, so subprocess with
...
# ---------------------------------------------------------------------------
# 01c legacy console code page (v0.45)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 04b v0.45 pre-flight guards, through the real CLI
# ---------------------------------------------------------------------------
...
def step_convert_and_validate(mode, scratch):
    ...
    # v0.45: the converters now warn loudly on a partial run. Treat that as
...
def step_grid_conversion(scratch):
    ...
    v0.45, and it exists because of a half-fix. The row-seam coverage hole
...
def step_edges(dirs):
    ...
    # v0.45: count the two seam KINDS separately. The v0.42 run measured two
...
def main():
    ...
    print("=" * 74)
    print(" dem2dged v0.45 release check  -  %s" % stamp())
    ...
    try:
        ...
        for mode in ("geo", "utm"):
            ...
            if ok:
                dirs.append(out)
        # v0.45: the 2 x 2 grid, so step 09b below can measure a ROW seam.
        # Steps 05 and 07 both produce a single tile row, which is why 09b
        # reported "0 row + 2 column seam(s)" on the v0.45 gate run.
