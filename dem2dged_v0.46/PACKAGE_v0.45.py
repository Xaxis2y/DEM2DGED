...
r"""One-shot packaging automation for the dem2dged v0.45 release.

Run this LAST -- after RELEASE_CHECK_v0.45.py reports ALL STEPS PASSED.

    conda activate DGED
    cd /d C:\Users\xaxis\Documents\ES_Project\dem2dged_v0.41
    python PACKAGE_v0.45.py
...
What it does, in order:
  ...
  4. Builds dem2dged_v0.45.zip (source release) and
     dem2dged_validate_v0.45.zip (validator-only bundle) by calling the
...
# v0.45: this used to read "cannot import dem2dged_lib (...). Are you in the
...
# `PACKAGE_v0.45.py` instead of `python PACKAGE_v0.45.py`: Windows then
...
for name in ("dem2dged_v%s.zip" % EXPECTED_VERSION,
    ...
    # v0.45: this used to test name.startswith("dem2dged_v"), and
    ...
    if name == "dem2dged_v%s.zip" % EXPECTED_VERSION:
        if not any("tests/" in n or "tests\\" in n for n in names):
            problems.append(
                ...
                "EXCLUDE_DIRS is stripping it again (v0.45 finding 2)" % name)
