# dem2dged v0.47 — Package Contents
...
| `dem2dged_package.py` | Zips the **source** release (`dem2dged_v0.47.zip`). |
...
| `RELEASE_CHECK_v0.47.py` | **The release gate.** Runs every layer against real GDAL and writes one log per step into `_release_check_logs/`. |
...
`dem2dged_validate_v0.47/` is the validator-only bundle laid out ready to
