...
def test_version_is_a_bare_major_minor_string():
    """VERSION must stay MAJOR.MINOR -- audit_pure.py section 7, the
    packaging scripts and the release-stage split all key off that shape."""
    assert re.fullmatch(r"\d+\.\d+", dl.VERSION), dl.VERSION
...
def test_every_module_declares_the_library_version_in_its_header(module_name):
    ...
    m = re.search(r"^#\s*Version:\s*(\d+\.\d+)\s*$", text, re.M)
