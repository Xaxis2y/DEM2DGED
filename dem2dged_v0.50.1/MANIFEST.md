# dem2dged v0.50.1 — Package Contents
...
| `PACKAGE_v0.50.1.py` | Release-orchestration wrapper: version-checks the environment, builds both release zips via the two packagers below, prints the final file list. |
...
| `RELEASE_CHECK_v0.50.1.py` | **The release gate.** Runs every layer against real GDAL and writes one log per step into `_release_check_logs/`. |
