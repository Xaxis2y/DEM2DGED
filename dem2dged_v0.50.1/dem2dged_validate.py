...
def build_parser():
    ...
    p.add_argument("--terrain-qa", choices=["none", "basic", "full"],
...
def main(argv=None):
    ...
    if args.terrain_qa != "none":
        if not args.src:
            print("WARNING: --terrain-qa requested without -src; skipped")
        else:
            try:
                from dem2dged_terrain import (run_terrain_qa, write_json,
                                               compliance_result)
                qa = run_terrain_qa(args.tile_folder, args.src,
                                    output_dir=os.path.join(args.tile_folder,
                                                            "validation"),
                                    resample=args.resample,
                                    full=(args.terrain_qa == "full"))
                # Profiles are deliberately conservative until an approving
                # authority supplies product-specific thresholds. The
                # informational profile reports values without inventing a
                # pass criterion; standard/strict are useful placeholders for
                # an operator-supplied policy file in the next release.
                limits = {} if args.compliance_profile == "informational" else {
                    "max_bias": 1.0 if args.compliance_profile == "strict" else 2.0,
                    "max_rmse": 3.0 if args.compliance_profile == "strict" else 5.0,
                    "max_p95": 5.0 if args.compliance_profile == "strict" else 8.0,
                    "max_max": 10.0 if args.compliance_profile == "strict" else 20.0,
                }
                ...
                print("WARNING: terrain QA failed: %s" % e)
