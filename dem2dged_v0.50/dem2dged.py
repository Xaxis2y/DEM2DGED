...
parser.add_argument("--terrain-qa", choices=["none", "basic", "full"],
    default="basic", metavar="LEVEL",
    help="Terrain-fidelity QA after conversion: none|basic|full (default: basic; "
         "full adds the 9-position +/-0.5-pixel sensitivity test)")
...
def _run_auto_validation(input_raster: str, output_folder: str,
                         ...
                         compliance_profile: str = "informational") -> None:
        ...
        if terrain_qa != "none":
            try:
                ...
                qa = run_terrain_qa(output_folder, input_raster,
                                    ...
                                    full=(terrain_qa == "full"))
                # Keep the profile visible even when the profile is
                # informational; threshold values are intentionally not
                # invented here until an authority approves them.
                qa["compliance_profile"] = compliance_profile
                limits = {} if compliance_profile == "informational" else {
                    "max_bias": 1.0 if compliance_profile == "strict" else 2.0,
                    "max_rmse": 3.0 if compliance_profile == "strict" else 5.0,
                    "max_p95": 5.0 if compliance_profile == "strict" else 8.0,
                    "max_max": 10.0 if compliance_profile == "strict" else 20.0,
                }
                qa["compliance"] = compliance_result(
                    rep.n_fail > 0, qa["metrics"], limits,
                    steep=qa.get("slope_bins"))
                write_json(qa, os.path.join(output_folder, "terrain_metrics.json"))
                logger.info("Terrain QA written: %s" %
                            os.path.join(output_folder, "terrain_metrics.json"))
            except Exception as e:
