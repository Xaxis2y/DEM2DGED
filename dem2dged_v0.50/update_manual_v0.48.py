...
"""
update_manual_v0.48.py -- bring DEM2DGED_User_Manual.docx up to v0.48.
...
WHAT IT CHANGES
---------------
  1. Cover block          "VERSION 0.40-beta" / "0.46-beta" / "0.47-beta"
                          -> "VERSION 0.48-beta"; date -> "August 2026"
  2. Page footer          same predecessor set -> "0.48-beta"
  ...
  4. Troubleshooting/FAQ  unchanged 3 entries from v0.46/v0.47, PLUS one
                          new FAQ entry: "Elevation errors vary a lot
  ...
  5. Version history      adds a row for v0.48 (cumulative back through
...
USAGE (Anaconda Prompt, dedicated environment -- never base):
    ...
    python update_manual_v0.48.py

Note the command form: "python update_manual_v0.48.py", not
"update_manual_v0.48.py". See dem2dged_env.py for why that matters.
"""
...
NEW_VERSION = "0.48-beta"
NEW_DATE = "August 2026"
PREDECESSOR_VERSIONS = ("0.47-beta", "0.46-beta", "0.40-beta")   # checked in this order
...
# No new package files were added this release (v0.48 is documentation-only),
# so nothing new is appended here -- kept so this script is still safe to run
# against a manual at any older predecessor state.
PACKAGE_ROWS = [
    ("dem2dged_compare.py",
