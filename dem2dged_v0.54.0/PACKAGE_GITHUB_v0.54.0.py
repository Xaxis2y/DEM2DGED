...
"""Build a focused GitHub/source ZIP for dem2dged v0.54.0.
...
from PACKAGE_v0.54.0.py, which preserves the broader archival source package.
"""
from pathlib import Path
import zipfile

ROOT = Path(__file__).resolve().parent
OUT = ROOT.parent / "dem2dged_v0.54.0_github.zip"
FILES = [
    "LICENSE", "README.md", "START_HERE.md", "QUICKSTART.html",
    "DEM2DGED_User_Manual.md", "REQUIREMENTS_COMPLIANCE_V0.54.0.md",
    "MANIFEST.md",
    "VERSION.txt", "VALIDATOR_VERSION.txt",
    "DEM2DGED_Compliance_Policy.json", "dem2dged_compliance.py",
    ...
    "dem2dged_validate_package.py", "PACKAGE_v0.54.0.py",
    "PACKAGE_GITHUB_v0.54.0.py", "RELEASE_CHECK_v0.54.0.py",
...
LOADER_FILES = [
    "DGED_Loader/DGED_Loader.pyt", "DGED_Loader/DGED_Load_Tool_script.py",
    "DGED_Loader/README.md", "DGED_Loader/ATBX_WIZARD_GUIDE.md",
    "DGED_Loader/VERSION.txt", "DGED_Loader/build_and_package.py",
...
TEST_FILES = ["tests/conftest.py", "tests/README.md", "tests/test_lib.py",
              ...
              "tests/test_terrain.py", "tests/test_compliance.py"]

missing = [p for p in FILES + LOADER_FILES + TEST_FILES if not (ROOT / p).is_file()]
if missing:
...
with zipfile.ZipFile(OUT, "w", zipfile.ZIP_DEFLATED) as zf:
    for rel in FILES + LOADER_FILES + TEST_FILES:
        zf.write(ROOT / rel, "dem2dged_v0.54.0/" + rel)

print("Created %s (%d files, %.2f MB)" %
       (OUT, len(FILES) + len(LOADER_FILES) + len(TEST_FILES),
        OUT.stat().st_size / 1048576.0))
