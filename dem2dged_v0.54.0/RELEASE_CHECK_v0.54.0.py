...
r"""Release gate for dem2dged v0.54.0 -- run this locally, return the logs.
...
---------------------------------------------------------------------
    conda activate DGED
    cd /d path\to\dem2dged_v0.54.0
    python RELEASE_CHECK_v0.54.0.py
...
def step_environment():
    ...
    try:
        ...
        lines.append("Use 'python RELEASE_CHECK_v0.54.0.py', not "
                     "'RELEASE_CHECK_v0.54.0.py'.")
...
def step_pytest():
    ...
    # Keep pytest's temporary tree inside the project. Managed Windows
    # environments can allow Python to create a user-Temp directory while
    # denying the subsequent scandir that pytest uses for numbered folders.
    # A unique local base also prevents stale global pytest folders from
    # turning an otherwise clean release gate into a permissions failure.
    pytest_tmp = tempfile.mkdtemp(prefix="_pytest_release_", dir=HERE)
    try:
        rc, out = run([sys.executable, "-m", "pytest", "-v", "--tb=short",
                       "-ra", "--basetemp", pytest_tmp])
    finally:
...
def main():
    ...
    print("=" * 74)
    print(" dem2dged v0.54.0 release check  -  %s" % stamp())
