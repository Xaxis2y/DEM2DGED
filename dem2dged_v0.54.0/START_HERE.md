...
v0.54.0 separates DGED structural compliance, source eligibility,
source-to-output conversion fidelity and independent-reference accuracy.
Missing accuracy evidence is `NOT_EVALUATED`, not `PASS`. Normal conversion is
unchanged; select `--terrain-qa mountain` (or the GUI mountain checkbox) only
when you need >20% slope, peaks/valleys and +/-0.5-post diagnostics. A target
level finer than the source is blocked by default because interpolation cannot
create compliant source detail. A declared vertical-datum conversion is also
blocked unless PROJ can construct the required non-ballpark geoid operation.
...
```batch
python PACKAGE_v0.54.0.py        :: -> dem2dged_v0.54.0.zip (one level up)
```
...
- Added a DGIWG level-specific compliance engine and consolidated JSON/text
  evidence report.
- Added independent-reference accuracy evaluation and explicit
  `NOT_EVALUATED` states.
- Added source-resolution enforcement and hash/runtime conversion manifests.
- Added optional mountain precision QA without changing default conversion.
- Changed automatic metadata accuracy text from “predicted” to the accurate
  “product-level goal; not measured” basis.
