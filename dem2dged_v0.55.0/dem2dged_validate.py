...
Usage:
    ...
    -max-diff METRES    Tolerance for the sample-window comparison vs the
                        source (H2 section). Options: 5.0 m (stricter, v0.45
                        default) or 10.0 m (standard, v0.46+ default;
                        recommended for steep terrain). Default: 10.0 m
    -resample ALG       Resampling algorithm used to produce the tiles
                        (near|bilinear|cubic|...). Must match conversion.
                        Default: bilinear
...
def check_tile(tif, rep, folder):
    ...
    driver_name = ds.GetDriver().ShortName if ds.GetDriver() else None
    if driver_name != "GTiff":
        rep.fail("%s: raster driver is %s, expected GeoTIFF" %
                 (base, driver_name), tile=base, cat="metadata")
    else:
        rep.ok("%s: GeoTIFF container" % base, tile=base, cat="metadata")
    try:
        with open(tif, "rb") as stream:
            signature = stream.read(4)
        byte_orders = {b"II*\x00": "little-endian", b"MM\x00*": "big-endian",
                       b"II+\x00": "little-endian BigTIFF",
                       b"MM\x00+": "big-endian BigTIFF"}
        byte_order = byte_orders.get(signature)
        if byte_order:
            rep.ok("%s: valid TIFF byte order/header (%s)" %
                   (base, byte_order), tile=base, cat="metadata")
        else:
            rep.fail("%s: invalid TIFF byte-order/header signature %r" %
                     (base, signature), tile=base, cat="metadata")
    except OSError as exc:
        rep.fail("%s: cannot inspect TIFF byte-order/header (%s)" %
                 (base, exc), tile=base, cat="metadata")
...
def _resample_display_name(resample):
    """Return a reader-friendly name for a GDAL resampling algorithm."""
    labels = {
    ...
    key = (resample or "").strip().lower()
    return labels.get(key, key or "the stated resampling method")


def _sample_window_explanation(resample):
    """Explain an H2 failure without falsely attributing it to Bilinear."""
    method = _resample_display_name(resample)
    return (
        "This delivery was produced with %s. In a spot-checked window, the "
        "largest tile-to-source elevation difference exceeds the allowed "
        "tolerance. The source was re-warped with the same method, so this "
        "is a failure for %s, not evidence that a different comparison "
        "candidate failed. Do not distribute this candidate. If this is a "
        "resampling comparison, select only a different candidate that has "
        "no validation FAIL; otherwise investigate tile alignment, source "
        "registration, and the selected resampling method before increasing "
        "-max-diff." % (method, method)
    )


def _explain_finding(kind, msg, resample=None):
    ...
    for needles, explanation in _EXPLAIN_RULES:
        if all(n in low for n in needles):
            if explanation is None:
                return _sample_window_explanation(resample)
            return explanation
    return None
...
def _dataset_card_html(ds):
  ...
  <div class="card">
    ...
    if resample:
        meta_bits.append("resampling <b>%s</b>" %
                         _esc(_resample_display_name(resample)))
    meta = " &nbsp;·&nbsp; ".join(meta_bits)
    ...
    for title, lines in sections:
        ...
        for kind, m in others:
            ...
            explain = _explain_finding(kind, m, resample=resample)
    ...
    if n_fail and resample:
        delivery_note = (
            "This %s candidate has one or more validation FAIL findings and "
            "is not recommended for delivery. A FAIL belongs only to this "
            "named resampling method; it does not mean that other separately "
            "validated comparison candidates failed." %
            _resample_display_name(resample)
        )
        note = (note + " " if note else "") + delivery_note
...
def run_validation(tile_folder, src=None, max_diff=10.0, verbose=False,
                    resample="bilinear"):
    ...
    ``max_diff``: tolerance in metres for sample-window comparison (section H2).
    Defaults to 10.0 m (v0.46) -- higher tolerance accommodates resampling
    artifacts on steep terrain. Use 5.0 m for stricter validation. v0.45
    used 5.0 m as default.
...
def build_parser():
    ...
    p.add_argument("-max-diff", "--max-diff", dest="max_diff", type=float,
        default=10.0, metavar="METRES",
        help="Tolerance for the sample-window comparison vs the source (H2). "
             "Options: 5.0 m (stricter, v0.45 default) or 10.0 m (standard, "
             "v0.46+ default; recommended for steep terrain, handles "
             "resampling artifacts from bilinear/cubic). Default: 10.0 m")
    ...
    p.add_argument("--terrain-qa", choices=["none", "basic", "full", "mountain"],
        default="none", help="Also write terrain_metrics.json and error rasters; "
             "full adds the nine-position +/-0.5-pixel sensitivity test; "
             "mountain also checks >20%% slopes, peaks and valleys")
    p.add_argument("--compliance-profile",
        choices=["informational", "standard", "strict"],
        default="informational", help="Terrain QA policy profile")
    p.add_argument("--reference-dem", default=None, metavar="INDEPENDENT_DEM",
        help="Independent terrain reference for accuracy evaluation. This is "
             "kept separate from the conversion source DEM")
    p.add_argument("--source-horizontal-accuracy", type=float, default=None,
        metavar="CE90_METRES", help="Independently established source CE90")
    p.add_argument("--source-vertical-accuracy", type=float, default=None,
        metavar="LE90_METRES", help="Independently established source LE90")
    p.add_argument("--reference-horizontal-ce90", type=float, default=None,
        metavar="METRES", help="Measured output CE90 against control/reference")
    p.add_argument("--reference-relative-vertical-90", type=float, default=None,
        metavar="METRES", help="Measured output relative vertical accuracy at 90%%")
    p.add_argument("--require-full-compliance", action="store_true",
        help="Return a non-zero status when the DGIWG report is FAIL or "
             "NOT_EVALUATED")
...
def write_dgiwg_compliance(tile_folder, report, source_path=None,
                           terrain_qa=None, independent_qa=None,
                           error_budget=None,
                           compliance_profile="informational",
                           source_horizontal_accuracy=None,
                           source_vertical_accuracy=None,
                           reference_horizontal_ce90=None,
                           reference_relative_vertical_90=None):
    """Write the consolidated DGIWG evidence report and return its data."""
    from dem2dged_compliance import (
        build_compliance_report, conversion_fidelity, external_accuracy,
        infer_product_level, nominal_source_gsd_m, read_conversion_manifest,
        source_eligibility, structural_compliance, write_compliance_text,
        write_consolidated_outputs,
        write_json as write_compliance_json)
    from dem2dged_terrain import compliance_thresholds, inspect_source

    level = infer_product_level(tile_folder)
    if level is None:
        raise RuntimeError("could not infer one DGED product level from tile names")
    manifest = read_conversion_manifest(tile_folder)
    parameters = manifest.get("parameters", {})
    source_hacc = (source_horizontal_accuracy if
                   source_horizontal_accuracy is not None else
                   parameters.get("source_horizontal_accuracy_90_m"))
    source_vacc = (source_vertical_accuracy if
                   source_vertical_accuracy is not None else
                   parameters.get("source_vertical_accuracy_90_m"))
    source_gsd = None
    inspected_vertical = None
    if source_path:
        source_info = inspect_source(source_path)
        latitude = (source_info.extent[1] + source_info.extent[3]) / 2.0
        source_gsd = nominal_source_gsd_m(
            source_info.pixel_size, source_info.horizontal_crs, latitude)
        inspected_vertical = source_info.vertical_crs
    source_section = source_eligibility(
        level, source_gsd, source_vertical_accuracy_90_m=source_vacc,
        source_horizontal_accuracy_90_m=source_hacc,
        source_vertical_datum=(parameters.get("source_vertical_epsg") or
                               inspected_vertical))
    conversion_section = conversion_fidelity(
        terrain_qa.get("metrics") if terrain_qa else None,
        compliance_thresholds(compliance_profile))
    mountainous = bool((independent_qa or terrain_qa or {}).get(
        "mountain_metrics", {}).get("predominant_slope_over_20_percent"))
    independent_section = external_accuracy(
        independent_qa.get("metrics") if independent_qa else None,
        level, mountainous=mountainous,
        relative_vertical_90_m=reference_relative_vertical_90,
        horizontal_ce90_m=reference_horizontal_ce90)
    compliance = build_compliance_report(
        level, structural_compliance(report.n_fail, report.n_warn),
        source_section, conversion_section, independent_section,
        provenance=manifest, error_budget=error_budget)
    validation_dir = os.path.join(tile_folder, "validation")
    write_compliance_json(
        compliance, os.path.join(validation_dir, "compliance_report.json"))
    write_compliance_text(
        compliance, os.path.join(validation_dir, "compliance_report.txt"))
    write_consolidated_outputs(
        compliance, validation_dir, terrain_qa=terrain_qa,
        independent_qa=independent_qa, error_budget=error_budget)
    return compliance
...
def main(argv=None):
    ...
    if args.terrain_qa != "none" and not args.src:
        parser.error("--terrain-qa requires -src SOURCE_DEM")
    ...
    if args.html_report:
        ...
        dataset = {"name": name, "folder": args.tile_folder, "src": args.src,
                   "rep": rep, "tiles": tiles, "resample": args.resample}
    ...
    independent_qa = None
    error_budget = None
    if args.terrain_qa != "none":
        try:
            from dem2dged_terrain import (compliance_result,
                                           compliance_thresholds,
                                           run_terrain_qa, write_json)
            qa = run_terrain_qa(args.tile_folder, args.src,
                                output_dir=os.path.join(args.tile_folder,
                                                        "validation"),
                                resample=args.resample,
                                full=(args.terrain_qa in ("full", "mountain")),
                                mountain=(args.terrain_qa == "mountain"),
                                error_threshold=args.max_diff)
            limits = compliance_thresholds(args.compliance_profile)
            qa["compliance_profile"] = args.compliance_profile
            qa["compliance"] = compliance_result(
                rep.n_fail > 0, qa["metrics"], limits,
                steep=qa.get("slope_bins"))
            write_json(qa, os.path.join(args.tile_folder, "validation",
                                        "terrain_metrics.json"))
            terrain_txt = os.path.join(args.tile_folder, "validation",
                                       "terrain_report.txt")
            if os.path.isfile(terrain_txt) and args.report:
                with open(args.report, "a", encoding="utf-8") as f, \
                        open(terrain_txt, encoding="utf-8") as tf:
                    f.write("\n\n" + tf.read())
            if qa["compliance"]["overall"] == "FAIL":
                qa_failed = True
                print("ERROR: terrain QA does not meet the %s compliance profile"
                      % args.compliance_profile)
        except Exception as e:
            qa_failed = True
            print("ERROR: terrain QA failed: %s" % e)

    # Independent terrain accuracy is intentionally a second comparison.
    # Reusing the conversion source here would merely measure fidelity and
    # would incorrectly turn agreement with an imperfect source into PASS.
    if args.reference_dem:
        try:
            from dem2dged_terrain import run_terrain_qa
            independent_qa = run_terrain_qa(
                args.tile_folder, args.reference_dem,
                output_dir=os.path.join(args.tile_folder, "validation",
            ...
            qa_failed = True
            print("ERROR: independent-reference QA failed: %s" % e)

    if args.reference_dem and args.src:
        try:
            from dem2dged_terrain import run_error_budget_qa
            error_budget = run_error_budget_qa(
                args.tile_folder, args.src, args.reference_dem,
                output_dir=os.path.join(args.tile_folder, "validation"),
                resample=args.resample)
        except Exception as e:
            qa_failed = True
            print("ERROR: three-part error-budget QA failed: %s" % e)

    compliance_overall = "NOT_EVALUATED"
    try:
        compliance = write_dgiwg_compliance(
            args.tile_folder, rep, source_path=args.src, terrain_qa=qa,
            independent_qa=independent_qa, error_budget=error_budget,
            ...
            reference_relative_vertical_90=args.reference_relative_vertical_90)
        compliance_overall = compliance["overall"]
        print("DGIWG compliance evidence: %s" % compliance_overall)
    except Exception as e:
        qa_failed = True
        print("ERROR: compliance report failed: %s" % e)

    if rep.n_fail or qa_failed:
        return 1
    if args.require_full_compliance and compliance_overall != "PASS":
        return 2
    return 0
