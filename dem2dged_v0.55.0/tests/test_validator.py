...
def test_terrain_qa_requires_an_original_source_dem():
    with pytest.raises(SystemExit) as exc:
        dv.main(["any_delivery_folder", "--terrain-qa", "basic"])
...
def test_sample_window_explanation_names_the_actual_resampling_method():
    message = "sample window (10,20): max |diff| 23.942 m > tolerance 10.00 m"
    explanation = dv._explain_finding("fail", message, resample="near")
    assert "Nearest Neighbor" in explanation
    assert "Bilinear" not in explanation
    assert "Do not distribute this candidate" in explanation


def test_html_report_marks_a_failed_resampling_candidate_as_not_deliverable():
    rep = dv.Report()
    rep.section("H2. Sample-window pixel difference (tiles vs re-warped source)")
    rep.fail("sample window (10,20): max |diff| 23.942 m > tolerance 10.00 m")
    html = dv.render_html_report([{
        "name": "test_1_nearest_neighbor",
        "src": "source.tif",
        "rep": rep,
        "tiles": [],
        "resample": "near",
    }])
    assert "resampling <b>Nearest Neighbor</b>" in html
    assert "Do not distribute this candidate" in html
    assert "not recommended for delivery" in html
