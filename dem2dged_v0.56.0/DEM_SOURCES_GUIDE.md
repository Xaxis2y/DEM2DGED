...
For compliance testing, resolution alone is not enough. Record the source's
horizontal CRS, vertical datum and independently established CE90/LE90. If the
vertical datum is not EGM2008, pass `--source-vertical EPSG_CODE`; v0.54.0
requires a real non-ballpark PROJ/geoid operation before conversion. For final
accuracy evidence, use a genuinely independent control DEM with
`--reference-dem`—never reuse the conversion source as its own reference.
