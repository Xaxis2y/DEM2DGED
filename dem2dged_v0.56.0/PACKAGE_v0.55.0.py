...
r"""One-shot packaging automation for the dem2dged v0.55.0 release.

Run this LAST -- after RELEASE_CHECK_v0.55.0.py reports ALL STEPS PASSED.

    conda activate DGED
    cd /d path\to\dem2dged_v0.55.0
    python PACKAGE_v0.55.0.py
...
What it does, in order:
  ...
  4. Builds dem2dged_v0.55.0.zip (source release) and
     dem2dged_validate_v0.55.0.zip (validator-only bundle) by calling the
...
EXPECTED_VERSION = "0.55.0"
HERE = os.path.dirname(os.path.abspath(__file__))
OUT_DIR = os.path.dirname(HERE)

SWEEP_DIRS = ("__pycache__", ".pytest_cache", "build", "dist",
              "_release_check_logs", ".mypy_cache")
SWEEP_DIR_PREFIXES = (".pytest_tmp",)
SWEEP_FILES = ("selftest_prefilter_log.txt",)
...
required_tests = ("conftest.py", "test_lib.py", "test_validator.py",
                  "test_converters.py", "test_terrain.py",
                  "test_compliance.py", "README.md")
...
for root, dirs, _files in os.walk(HERE, topdown=True):
    for d in list(dirs):
        if d in SWEEP_DIRS or d.startswith(SWEEP_DIR_PREFIXES):
...
for name in ("dem2dged_v%s.zip" % EXPECTED_VERSION,
    ...
    scratch = [n for n in names
               ...
               or "__pycache__" in n or "/.pytest_tmp" in n
               or n.lower().endswith("selftest_prefilter_log.txt")]
    if scratch:
