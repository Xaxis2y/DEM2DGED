# DEM2DGED v0.56.0 Requirements and Compliance Evidence
...
## v0.56.0 review corrections

A full-project review of v0.55.0 found, and this release fixes, seven
correctness defects. Three of them affected delivered product directly, so
they are recorded here as evidence rather than only in `VERSION.txt`. Each is
covered by a test in `tests/test_v056_regressions.py` that fails against
v0.55.0.

| Requirement area | Defect in v0.55.0 | Status in v0.56.0 |
|---|---|---|
| Avoid interpolation for an identical grid | `try_direct_copy_tile()` compared a DGED post against GDAL's pixel-CORNER geotransform. A correctly point-registered source was always rejected, and a corner-aligned source was accepted and written half a post out of place. | Fixed; both directions asserted by test. The direct-copy path was previously untested. |
| DGED structure/header/metadata | Sidecar values were substituted without XML escaping, so a source filename containing `&` produced malformed XML for every tile. | Fixed at the write boundary with the existing `_xml_escape()`. |
| DGED structure/header/metadata | The four XML writers used the platform code page while declaring `encoding="UTF-8"`; on cp1252 a non-ASCII lineage aborted the conversion mid-run. | Fixed; the validator now reports a mis-encoded sidecar as a finding instead of crashing. |
| Source CRS / extent | Extent reprojection sampled only four corners, losing 4116 m of coverage on a 6x5 degree extent at 55-60N. Tiles reachable only from that strip were never generated. | Fixed; both bbox transforms densify each edge with 21 points, matching GDAL's `SuggestedWarpOutput`. |
| Product statistics and completeness | A NaN NoData sentinel made a good tile report `(0, 0, 100.0)`, and the anti-alias pre-filter spread one NaN post into 81. | Fixed by one shared `valid_data_mask()` predicate. |
| Vertical/CRS selection | Svalbard UTM auto-detect split on the wrong longitudes; 7 of 9 probe longitudes resolved to the wrong zone, i.e. the wrong CRS for the delivery. | Fixed to the spec boundaries; the warning is now printed rather than hidden behind `-verbose`. |
| Compliance decision model | A profile absent from the policy file silently meant "no thresholds", and an all-INFO result reported overall `PASS`. | Fixed: missing profiles fall back to the bundled defaults, and a run that evaluated nothing reports `NOT_EVALUATED`, consistent with the decision model above. |
| Automated verification | The GUI is a second converter implementation with no test coverage; the `DGED_Loader` harness never ran under pytest. | Both now covered. `RELEASE_GATE_v0.56.0.py` runs the whole gate in one command. |


## Requirement matrix

| Requirement area | v0.56.0 implementation | Evidence/status |
|---|---|---|
