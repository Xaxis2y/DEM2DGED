...
def validate_prefilter(name: Optional[str]) -> str:
    ...
    raise SystemExit(
        ...
        "        to suppress aliasing when downsampling high-relief terrain.)"
...
def resolve_resampler(input_path: str, src_gsd_m: float, dst_gsd_m: float,
    ...
    source raster (four cheap in-memory warps, no tiles written, no HTML
    report) and returns whichever of Nearest / Bilinear / Cubic / Cubic
    B-Spline reconstructs withheld source posts most accurately for THIS
    input -- cubic-family candidates are scored on the same source-range
    clamp their delivered tiles would actually get, so overshoot at a few
    sharp edges no longer unfairly penalizes them (see
    dem2dged_compare._holdout_stats()'s docstring). It costs extra time
    (roughly one read of the source plus four small warps) that "auto" does
