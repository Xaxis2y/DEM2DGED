...
code_keys = {"BASENAME", "LEVEL", "GSD", "DATE", "EPSG", "ORG", "CLASS_WORD",
             ...
             "ABS_HACC", "ABS_VACC", "ABS_HACC_BASIS", "ABS_VACC_BASIS",
             "LINEAGE", "DTYPE"}
...
#    and (v0.57) the caller's dst_gsd_m through as-is, so "optimize" holds
#    out at the actual requested ratio instead of a fixed 2x -- see
#    dem2dged_v0.56.0_mountain_terrain_review.md for why that regressed
#    accuracy on steep, high-ratio conversions before this fix.
...
dc.pick_best_resampling = (lambda src, angular=False, log_fn=None, dst_gsd_m=None:
    (_dispatch_calls.append((src, angular, dst_gsd_m)),
     ("cubic", "Cubic Convolution", {}))[1])
try:
    r1 = dl.resolve_resampler("mydem.tif", 10, 40, "optimize")
    r2 = dl.resolve_resampler("aspect.tif", 10, 40, "OPTIMIZE")  # case-insensitive
    check(r1 == "cubic" and r2 == "cubic",
          "resolve_resampler('optimize') should return pick_best_resampling's choice")
    check(_dispatch_calls == [("mydem.tif", False, 40), ("aspect.tif", True, 40)],
          "resolve_resampler('optimize') passed the wrong (src, angular, "
          "dst_gsd_m) tuple(s) through to pick_best_resampling: %r"
          % (_dispatch_calls,))
finally:
...
#    among the five candidates (v0.57: was three, then four with cubicspline,
#    now five with average -- see AUTO_OPTIMIZE_CANDIDATES), and falls back
#    to Bilinear (never crashes) if every candidate's warp fails. Both
#    _read_source and _holdout_stats are monkeypatched so this needs no real
#    raster / GDAL Warp() call. ----------------------------------------------
#
# v0.57: _read_source's mock used to return the placeholder strings "ARR" /
# "CGT" for the array and geotransform. pick_best_resampling() now reads
# arr.shape (to size pick_holdout_factor()'s minimum-training-grid check)
# and cgt[1] (the source pixel size, to compute the requested ratio when
# dst_gsd_m is given) UNCONDITIONALLY, even when dst_gsd_m is None -- so a
# bare string placeholder for either one now breaks this mock with an
# AttributeError/TypeError before pick_best_resampling can be exercised at
# all. Replaced with values that behave like the real thing for exactly the
# operations pick_best_resampling performs on them, without needing a real
# GDAL raster.
class _FakeArr:
    shape = (200, 200)

_FAKE_CGT = (500000.0, 5.0, 0.0, 5600000.0, 0.0, -5.0)  # 5 m source posts

dc._read_source = (lambda src:
    (_FakeArr(), "VALID", _FAKE_CGT, "PROJ", "NODATA", 1))
try:
    _rmse_by_alg = {"near": 5.0, "bilinear": 1.2, "cubic": 1.8,
                    "cubicspline": 2.5, "average": 1.5}
    dc._holdout_stats = (
        lambda arr, valid, cgt, proj, nodata, alg, holdout_factor=2:
            {"rmse": _rmse_by_alg[alg], "mae": _rmse_by_alg[alg] * 0.8,
             "n_holdout": 1000, "holdout_factor": holdout_factor})
    ...
    check(set(stats) == {"near", "bilinear", "cubic", "cubicspline", "average"},
          "stats_by_alg should report all 5 candidates, got %r" % (set(stats),))
    check(all(st["holdout_factor"] == 2 for st in stats.values()),
          "no dst_gsd_m given -- every candidate should have been held out "
          "at the original fixed 2x, got %r"
          % {a: st["holdout_factor"] for a, st in stats.items()})
    ...
    # v0.57: dst_gsd_m=200 against the 5 m source above is a 40x decimation
    # (clamped to MAX_HOLDOUT_FACTOR=20) -- the ratio-aware fix's whole
    # point is that "optimize" now tests THIS ratio, not a fixed 2x.
    _rmse_by_alg["average"] = 0.05   # now average is the most accurate
    alg2b, _label2b, stats2b = dc.pick_best_resampling(
        "dem2b.tif", angular=False, dst_gsd_m=200.0)
    check(alg2b == "average",
          "lowest-RMSE candidate (average=0.05) should win, got %r" % (alg2b,))
    check(all(st["holdout_factor"] == dc.MAX_HOLDOUT_FACTOR
              for st in stats2b.values()),
          "dst_gsd_m=200 on a 5 m source (40x, capped) should hold out at "
          "MAX_HOLDOUT_FACTOR=%d, got %r" % (dc.MAX_HOLDOUT_FACTOR,
                                             {a: st["holdout_factor"]
                                              for a, st in stats2b.items()}))
