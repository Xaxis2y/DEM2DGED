...
def compute_method_stats(src_path, method_folder, alg="bilinear"):
    ...
    Returns a dict of metrics (all elevation units are metres):
      rmse / mae / max_abs_err / n_holdout /
...
def pick_best_resampling(src_path, angular=False, log_fn=None, dst_gsd_m=None):
    ...
    if known. Used with the source GSD read off ``src_path`` to compute the
    actual decimation ratio and hold out at (approximately) that ratio via
    pick_holdout_factor(), instead of the fixed 2x test every version before
    v0.57 always ran. This matters because which algorithm reconstructs
    terrain most accurately is RATIO-DEPENDENT: a controlled measurement
    ...
    if dst_gsd_m:
        src_px = abs(cgt[1])
        if src_px > 0:
            ratio = float(dst_gsd_m) / src_px
    holdout_factor = pick_holdout_factor(ratio, arr.shape)

    if ratio and holdout_factor != 2:
        _log("Auto-optimize: running hold-out accuracy comparison "
             "(Nearest / Bilinear / Cubic / Cubic B-Spline / Average) "
             "against the source DEM, held out at %dx (the actual "
             "requested decimation is ~%.1fx)..." % (holdout_factor, ratio))
    else:
        _log("Auto-optimize: running hold-out accuracy comparison "
             "(Nearest / Bilinear / Cubic / Cubic B-Spline / Average) "
             "against the source DEM, held out at the standard 2x test...")
...
def write_comparison_report(entries, html_path):
    ...
    tr.best td.method::after { content:"  \\2605  Best Hold-out";
    ...
    for e in entries:
        ...
        parts.append(
            ...
            "<th rowspan='2'>Validation</th><th rowspan='2'>Hold-out Rank</th>"
            "</tr>"
            "<tr>"
        ...
        if best:
            parts.append(
                "<div class='summary'>Best hold-out reconstruction: "
