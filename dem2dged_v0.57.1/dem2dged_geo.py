...
parser.add_argument("-resample", dest="resample", default="auto",
    help="Resampling: auto|optimize|bilinear|cubic|cubicspline|average|"
         ...
         "bilinear; optimize = measure Nearest/Bilinear/Cubic/Cubic "
         "B-Spline/Average against the source DEM AT THE ACTUAL DECIMATION "
         "RATIO and use whichever reconstructs it most accurately -- "
         "slower, see dl.resolve_resampler(); 'rms' is deliberately not "
         "offered -- it discards sign for signed elevation data, use "
         "'average' instead)")
parser.add_argument("-prefilter", dest="prefilter", default="none",
    help="Anti-alias pre-filter applied to the SOURCE before warping: "
         ...
         "in as false structure. MEASURED (v0.57): on steep terrain this "
         "reduced net point-accuracy at every ratio/resampler tested, by "
         "44-102%% -- NOT currently recommended for improving accuracy on "
         "mountainous sources. See dl.build_prefiltered_source().")
...
def main(args):
    ...
    #
    # Deliberately NOT applied to the clamp range computed above: that must
    # stay the ORIGINAL source's true min/max, because it exists to catch a
    # resampler inventing a physically impossible elevation, and smoothing
    # narrows the range rather than widening it.
    warp_input     = pargs.input_raster
    prefilter_tmp  = None
    prefilter_note = "no anti-alias pre-filter"
    if pargs.prefilter == "gaussian":
        sigma_px = dl.gaussian_sigma_for_ratio(src_gsd_m, dst_gsd_m,
    ...
    # Direct copy is safe only for an explicitly point-registered source
    # already in the target reference and with no prefilter. Otherwise the
    # normal warp path remains authoritative.
    ...
    for yy in range(ilat_s, ilat_e):
        for xx in range(ilon_s, ilon_e):
            ...
            if os.path.isfile(xml_path) and os.path.isfile(tif_path):
            ...
            if direct_copy_allowed:
                nx = int(round(tiledim / lonres)) + 1
                ny = int(round(tiledim / latres)) + 1
                if dl.try_direct_copy_tile(
                        pargs.input_raster, tif_path,
                        first_x=t_minlon, first_y=t_minlat + tiledim,
                        width=nx, height=ny, xres=lonres, yres=latres,
                        dst_srs="EPSG:%s+3855" % MY_OUT_SRS,
                        out_type=out_type):
                    direct_copy_used = True
                    dl.dp("Direct-copy grid match: %s" % basename)
                    dl.fix_header(tif_path,
                                  "EPSG:%s+3855" % MY_OUT_SRS)
                    tile_grid[(yy, xx)] = tif_path
                    pending.append(dict(basename=basename, tif_path=tif_path,
                                         xml_path=xml_path, pw=pw, ps=ps,
                                         pe=pe, pn=pn))
                    tile_basenames.append(basename)
                    _note_delivered(pw, ps, pe, pn)
                    print("%d%% done" % pct)
    ...
    # raster (which can be several hundred MB in the system temp folder) is
    # no longer needed. Removed here, BEFORE the no-tiles SystemExit below,
    # so a failed run cleans up after itself too.
