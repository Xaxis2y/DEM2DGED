...
def valid_data_mask(arr, nodata):
    ...
    routine in Float32 DEMs:
      ...
      compute_tile_stats()      abs(arr - nodata) > 0.5
          ...
          assumed -- the v0.56.0 review harness reproduced exactly that.
...
#
# v0.57: "rms" removed. gdalwarp's -r rms computes the QUADRATIC MEAN
# (sqrt(mean(x^2))) of each output pixel's contributing footprint. That is
# mathematically guaranteed to be >= |true mean| for any footprint with
# variation -- it is not an unbiased estimator of elevation, and for any
# footprint straddling a vertical datum (or, at scale, any footprint with
# real local relief) it can be badly and silently wrong. MEASURED: a 4x4
# footprint with true mean -50.0 m produced +100.0 m under "rms" -- 150 m
# off, and the wrong SIGN, with gdalwarp reporting success. This is
# documented, standard GDAL behaviour (added 3.3, intended for magnitude
# quantities like backscatter, not signed physical elevation) rather than a
# dem2dged bug, but nothing warned a user who tried it -- and "-resample
# optimize" actively encourages experimenting with resamplers. See
# REMOVED_RESAMPLERS below for the message a caller who still asks for it
# gets.
GDALWARP_RESAMPLERS = ("near", "bilinear", "cubic", "cubicspline",
                       "lanczos", "average", "min", "max", "med",
                       "q1", "q3")
META_RESAMPLERS = ("auto", "optimize")
VALID_RESAMPLERS = frozenset(GDALWARP_RESAMPLERS + META_RESAMPLERS)

# v0.57: resamplers that used to be accepted (or that sound plausible) but
# are unsound for signed elevation data, mapped to a specific explanation.
# Checked before the generic "unknown resampling method" error in
# validate_resampler() so a user who asks for one of these gets the REASON,
# not just a value that happens to no longer be in VALID_RESAMPLERS.
REMOVED_RESAMPLERS = {
    "rms": (
        "'rms' (root-mean-square / quadratic mean) discards the SIGN of "
        "every value it averages -- sqrt(mean(x^2)) is never less than "
        "|mean(x)|, so it is never an unbiased estimate of elevation. On "
        "real terrain (or any footprint straddling a vertical datum) this "
        "can be wrong by tens to hundreds of metres with gdalwarp reporting "
        "success. Use 'average' instead -- it is the true arithmetic mean "
        "of the footprint and is what -resample auto already selects when "
        "downsampling."),
}
...
def validate_resampler(name: Optional[str]) -> str:
    ...
    list of valid values for anything else -- with a specific explanation
    first for any name in REMOVED_RESAMPLERS (v0.57), since "unknown value,
    here is the list" is much less useful than "this value is unsound and
    here is why" for a resampler gdalwarp itself would happily accept.
    None/"" resolves to "auto", the documented default.
    """
    n = (name or "auto").strip().lower()
    if n in VALID_RESAMPLERS:
        return n
    if n in REMOVED_RESAMPLERS:
        raise SystemExit(
            "ERROR: -resample %s is not offered by dem2dged.\n"
            "       %s" % (n, REMOVED_RESAMPLERS[n]))
    raise SystemExit(
        "ERROR: unknown resampling method '%s'.\n"
        "       Valid values are: %s\n"
        "       ('auto' = average when downsampling, else bilinear; "
        "'optimize' = measure\n"
        "        Nearest/Bilinear/Cubic/Average against this DEM at the "
...
#
# "none" is the default and reproduces every pre-v0.49 delivery byte for
# byte. "gaussian" applies a low-pass filter to the SOURCE raster before
# any tile is warped -- see build_prefiltered_source() for why that is a
# separate pass rather than a gdalwarp flag.
#
# v0.57 CORRECTION: this feature was documented as an accuracy improvement
# for high-relief/mountainous downsampling ("suppress aliasing"). A
# controlled measurement against known ground truth (synthetic terrain with
# realistic mountain slope statistics, exact elevation known at every DGED
# post -- see dem2dged_v0.56.0_mountain_terrain_review.md) found the
# OPPOSITE: turning it on increased point-wise vertical error at every
# ratio (4x/8x/16x) and every resampler tested (Average/Bilinear/Cubic), by
# 44-102%, and the penalty grows with the decimation ratio -- i.e. it is
# worst exactly on the steep, high-ratio conversions it was written for.
# Mechanism: average/bilinear/cubic already blend multiple source samples
# per output pixel, which is itself a low-pass operation; stacking a second,
# independent Gaussian low-pass in front of that over-smooths the terrain.
# The reduction in aliasing "ringing" it does produce is smaller than the
# smoothing bias it adds, when scored against the DGED spec's actual
# criterion (the elevation AT the post). It may still reduce large-scale
# visual banding/moire in a hillshade or overview -- that was not measured
# either way -- so the feature is kept, opt-in, exactly as before; only the
# claim that it improves point accuracy has been removed from the CLI/GUI
# help text and this docstring.
VALID_PREFILTERS = frozenset({"none", "gaussian"})


def validate_prefilter(name: Optional[str]) -> str:
    """Normalise and check a -prefilter / GUI pre-filter value (v0.49).

    Mirrors validate_resampler()'s contract exactly: returns the
    lower-cased, stripped name, raises SystemExit listing the valid values
    for anything else, and resolves None/"" to the documented default --
    which for the pre-filter is "none", NOT an active filter. The v0.49
    feature is strictly opt-in: a command line that worked before v0.49
    produces bit-identical tiles after it.

    v0.57: the error message's description of 'gaussian' no longer claims
    it improves accuracy on high-relief terrain -- see the CORRECTION note
    on VALID_PREFILTERS above for the measurement that retracted the claim.
    """
    n = (name or "none").strip().lower()
    if n in VALID_PREFILTERS:
        return n
    raise SystemExit(
        "ERROR: unknown pre-filter '%s'.\n"
        "       Valid values are: %s\n"
        "       ('none' = no pre-filtering, the default and the pre-v0.49\n"
        "        behaviour; 'gaussian' = low-pass the source before warping.\n"
        "        Reduces aliasing 'ringing', but MEASURED to increase net\n"
        "        point-accuracy error on steep terrain -- see\n"
        "        dem2dged_v0.56.0_mountain_terrain_review.md before using it\n"
        "        to try to improve accuracy on mountainous sources.)"
        % (name, ", ".join(sorted(VALID_PREFILTERS))))


def gaussian_sigma_for_ratio(src_gsd_m: float, dst_gsd_m: float,
                             override: Optional[str] = "auto") -> float:
    """Gaussian sigma, IN SOURCE PIXELS, for the anti-alias pre-filter (v0.49).

    THE PROBLEM. Resampling a DEM to a coarser post spacing is decimation,
    and decimation without a low-pass filter first is textbook aliasing:
    terrain detail with a wavelength shorter than twice the TARGET post
    spacing cannot be represented at the target spacing, so instead of
    disappearing cleanly it folds back into the output as spurious
    long-wavelength structure. On flat terrain there is little such
    short-wavelength energy and the effect is invisible. On mountainous
    terrain -- ridge lines, gully walls, cliff edges -- there is a great
    deal of it, which is a large part of why a fixed DGED level degrades
    so much faster on high relief (see the v0.48 documentation pass).

    THE SIGMA. With a decimation ratio r = dst_gsd / src_gsd, the standard
    anti-aliasing choice used by image-pyramid implementations (this is
    scikit-image's transform.resize(anti_aliasing=True) rule) is

        sigma = (r - 1) / 2      [source pixels]

    It has the right limiting behaviour at both ends: at r = 1 (no
    resolution change) it is exactly 0, so the filter is a no-op rather
    than a gratuitous smoothing pass, and it grows linearly as the target
    spacing coarsens. For r <= 1 -- UPSAMPLING, where the target grid is
    finer than the source and there is no aliasing to prevent -- it is
    clamped to 0.0 and the caller is expected to skip the filter entirely.

    ``override``: "auto" (or None/"") applies the formula above. Anything
    else is parsed as an explicit sigma in SOURCE PIXELS, which is how a
    user tunes the strength by hand -- 0 disables, larger smooths harder.
    A negative or unparseable value is a SystemExit, not a silent fallback,
    because a mistyped sigma would otherwise quietly change every delivered
    elevation.

    v0.57 CAUTION -- this sigma is textbook-correct for reducing aliasing
    RINGING, but a controlled measurement against known ground truth found
    that applying it before average/bilinear/cubic resampling (which are
    themselves already a low-pass operation) increases net point-accuracy
    error on steep terrain at every ratio and resampler tested -- see the
    CORRECTION note above VALID_PREFILTERS. Kept as-is because it is still
    the right formula IF a caller has an independent reason to want this
    exact filter strength; it is no longer treated anywhere in this project
    as the accuracy-improving default it was believed to be.

    Returns the sigma as a float. 0.0 means "do not filter".
    """
    ov = (override if override is not None else "auto")
    ...
    if ov not in ("", "auto"):
        try:
            ...
            raise SystemExit(
                "ERROR: -prefilter_sigma must be 'auto' or a number in "
                "source pixels, got '%s'." % override)
        if sigma < 0:
            raise SystemExit(
                "ERROR: -prefilter_sigma must be >= 0, got %s." % override)
        return sigma

    if not src_gsd_m or src_gsd_m <= 0 or not dst_gsd_m or dst_gsd_m <= 0:
        return 0.0
...
def _gaussian_kernel_1d(sigma: float, truncate: float = 4.0):
    """Normalised 1-D Gaussian kernel, as a numpy array of odd length.

    Truncated at ``truncate`` standard deviations (4.0, the same default
    scipy.ndimage.gaussian_filter uses), and explicitly re-normalised to
    sum to 1.0 AFTER truncation, so the filter preserves the mean
    elevation of the terrain it passes over. Without that re-normalisation
    the discarded tails would bias every filtered height slightly toward
    zero.
    """
    import numpy as np
...
def _convolve1d(a, k, axis: int):
    """Convolve a 2-D array with a 1-D kernel along one axis, edge-padded.

    Separable convolution done as a shift-and-accumulate over the kernel
    taps: for a radius-R kernel this is 2R+1 whole-array multiply-adds,
    which numpy vectorises, rather than a Python loop over pixels. Edge
    padding (repeat the border value) is the right choice for terrain --
    it extrapolates the boundary as locally flat instead of mirroring a
    slope back on itself.

    Deliberately implemented in pure numpy rather than calling
    scipy.ndimage.gaussian_filter: numpy is already required by this
    project (compute_tile_stats, clamp_tile_to_range, dem2dged_compare),
    scipy is NOT, and adding a heavyweight dependency to the packaged .exe
    for one 15-line function is a poor trade.
    """
    import numpy as np
...
def build_prefiltered_source(input_path: str, sigma_px: float,
                             out_path: Optional[str] = None,
                             log_fn=print,
                             max_block_bytes: int = 256 * 1024 * 1024) -> str:
    """Write a Gaussian-smoothed copy of a source DEM, for anti-aliasing (v0.49).

    Returns the path to a new Float32 GeoTIFF carrying the SAME geotransform,
    projection and dimensions as ``input_path``, with band 1 low-pass
    filtered by a Gaussian of ``sigma_px`` source pixels. Both converters
    then hand that file to gdalwarp instead of the original, so the tile
    geometry, the resampler choice and every other step downstream are
    completely unchanged -- this is a pre-processing pass, not a change to
    the conversion.

    WHY A SEPARATE PASS, NOT A gdalwarp FLAG. gdalwarp has no low-pass
    option. Its "-r average" comes closest and is what "-resample auto"
    already picks when downsampling, but average is a BOX filter over each
    output pixel's own footprint: its frequency response has large side
    lobes, so it leaks exactly the short-wavelength energy this is meant
    to suppress, and it cannot be tuned. A Gaussian is separable, has no
    side lobes, and its width is set independently of the output grid.

    NODATA IS HANDLED BY NORMALISED CONVOLUTION, NOT IGNORED. A plain
    convolution over an array holding -32767 in the voids would smear that
    sentinel hundreds of metres into real terrain along every void margin
    and coastline. Instead both the data (with voids zeroed) and the
    validity mask are filtered, and the result is their ratio -- the
    standard normalised-convolution (Nadaraya-Watson) estimator, which
    weights each output by only the VALID neighbours that contributed.
    Pixels that were NoData in the source are then restored to NoData
    exactly, so the void footprint is preserved to the pixel and no
    interpolated value is ever invented inside one.

    OUTPUT IS ALWAYS Float32, even for an Int16 source: smoothing produces
    fractional heights, and rounding them back to whole metres here would
    quantise twice (once now, once at the converter's "-ot Int16"). The
    converter's own -ot still governs the delivered tile's type.

    MEMORY. Processed in horizontal strips sized to ``max_block_bytes``
    (256 MB of working arrays by default), each read with a ``radius``-row
    halo above and below so the filter output is identical to a whole-array
    pass -- there is no seam at a strip boundary. A multi-gigabyte source
    is therefore fine.

    ``out_path``: where to write. Defaults to a path in the system temp
    directory; callers own it and are expected to delete it once the last
    tile is warped (see cleanup_prefiltered_source()).

    Raises SystemExit with an actionable message if the source cannot be
    opened or the output cannot be created.
    """
    import numpy as np

    src = gdal_open(input_path)
    if src is None:
        raise SystemExit("ERROR: GDAL cannot open the source raster for "
                         "pre-filtering: %s" % input_path)
    ...
    if out_path is None:
        import tempfile
        fd, out_path = tempfile.mkstemp(prefix="dem2dged_prefilter_",
                                        suffix=".tif")
        os.close(fd)
        os.remove(out_path)
    ...
    if dst is None:
        src = None
        raise SystemExit("ERROR: cannot create the pre-filtered scratch "
                         "raster: %s" % out_path)
    dst.SetGeoTransform(src.GetGeoTransform())
    dst.SetProjection(src.GetProjection())
    dband = dst.GetRasterBand(1)
    if nodata is not None:
        dband.SetNoDataValue(float(nodata))

    # 4 float64 working arrays per strip (data, mask, and the two
    # convolution accumulators), plus the halo rows.
    row_bytes = max(1, nx * 8 * 4)
    rows_per_block = max(1, int(max_block_bytes // row_bytes))

    log_fn("Pre-filter: Gaussian sigma=%.3f source px (kernel radius %d), "
           "%d row(s) per block" % (sigma_px, radius, rows_per_block))

    y = 0
    while y < ny:
        h = min(rows_per_block, ny - y)

        # Read this strip PLUS whatever halo actually exists in the raster,
        # then edge-pad only the part the raster boundary truncated. Output
        # rows [radius : radius+h] of the padded strip then depend solely on
        # real data, so a strip boundary is not a seam.
        ry0 = max(0, y - radius)
        ry1 = min(ny, y + h + radius)
        arr = band.ReadAsArray(0, ry0, nx, ry1 - ry0).astype("float64")
        ...
        num = _convolve1d(_convolve1d(data0, k, 0), k, 1)
        den = _convolve1d(_convolve1d(valid, k, 0), k, 1)
        ...
        dband.WriteArray(out[radius:radius + h, :].astype("float32"), 0, y)
        y += h
...
def cleanup_prefiltered_source(path: Optional[str], log_fn=None) -> None:
    """Delete a build_prefiltered_source() scratch file, never raising.

    The scratch raster lives in the system temp directory and can be
    several hundred megabytes, so it is removed as soon as the last tile
    has been warped rather than left for the OS. Failure to delete is not
    worth aborting a finished conversion over -- the tiles are already on
    disk -- so it is swallowed, optionally logged.
    """
    if not path:
        return
    try:
        ...
        if log_fn:
            log_fn("NOTE: could not remove the pre-filter scratch file %s "
                   "(%s); it is safe to delete by hand." % (path, exc))
...
#
# Changelog:
#   0.50.3 Release-verification correction: complete three-part version
#         metadata, reliable source-CRS inspection, policy-file-backed
#         terrain compliance gates, and DGED-only QA tile discovery. The
#         integrated DGED_Loader excludes source and QA rasters when scanning
#         a delivery parent folder.
#   0.49  OPT-IN GAUSSIAN ANTI-ALIAS PRE-FILTER for high-relief source data
#         (-prefilter / -prefilter_sigma, default "none" = the pre-v0.49
#         behaviour, bit for bit). Downsampling a DEM to a coarser DGED
#         post spacing is decimation, and decimation without a low-pass
#         filter first aliases: terrain detail shorter than twice the
#         TARGET spacing folds back into the product as spurious
#         long-wavelength structure instead of disappearing cleanly.
#         Mountainous sources carry far more of that short-wavelength
#         energy than flat ones, which is part of why a fixed level
#         degrades faster on high relief (see the v0.48 doc pass).
#         New in this module: VALID_PREFILTERS / validate_prefilter(),
#         gaussian_sigma_for_ratio() (sigma = (r-1)/2 source pixels, the
#         standard image-pyramid rule, 0 when not downsampling),
#         build_prefiltered_source() and cleanup_prefiltered_source().
#         Three things worth knowing about the implementation:
#         (1) It is a SEPARATE PASS over the source, not a gdalwarp flag,
#             because gdalwarp has no low-pass option -- "-r average" is a
#             box filter with large side lobes and no tunable width.
#             Everything downstream (tile geometry, -te/-tr snapping, the
#             resampler, edge reconciliation) is untouched.
#         (2) NoData is handled by NORMALISED convolution: filtering a
#             raster holding -32767 in its voids would otherwise smear that
#             sentinel deep into real terrain along every void margin and
#             coastline. Void footprints are restored exactly afterwards.
#         (3) It is a BIAS/VARIANCE TRADE, not free accuracy -- smoothing
#             lowers RMSE against a resampled reference while systematically
#             clipping peaks and filling valleys. That is why it is opt-in
#             and off by default, and why the docs tell operators to verify
#             with dem2dged_validate.py on their own data rather than
#             assume it helps. New selftest_prefilter.py measures both
#             sides of that trade on synthetic ridged terrain.
...
# off, so it uses the complete "MAJOR.MINOR.PATCH" release number.
# RELEASE_STAGE carries an optional qualifier separately; VERSION_DISPLAY is
# what the user sees in --version, the GUI title bar and report headers.
RELEASE_STAGE = ""
...
# Used as the DEFAULT target-goal value written to the metadata quality
# reports when the operator does not supply measured values. The XML labels
# these values explicitly as goals, never as measured or predicted accuracy.
LEVEL_ABS_HACC = {
...
def sidecar_replacements(basename: str, level: str, gsd: float, epsg: str,
    ...
    def _acc(value, table, axis):
        if value is None or str(value).strip().lower() in ("", "auto"):
            return (table.get(level, ""),
                    "DGIWG 250 product-level %s accuracy goal; not measured "
                    "or independently evaluated" % axis)
        return (value,
                "Operator-supplied %s accuracy value; see lineage and "
                "external evidence for its verification basis" % axis)

    hacc, hacc_basis = _acc(abs_hacc, LEVEL_ABS_HACC,
                            "horizontal CE90")
    vacc, vacc_basis = _acc(abs_vacc, LEVEL_ABS_VACC,
                            "vertical LE90")
    ...
    return {
        ...
        "ABS_HACC":  hacc,
        "ABS_VACC":  vacc,
        "ABS_HACC_BASIS": hacc_basis,
        "ABS_VACC_BASIS": vacc_basis,
...
def try_direct_copy_tile(src_path: str, dst_path: str, *,
                         first_x: float, first_y: float,
                         width: int, height: int,
                         xres: float, yres: float,
                         dst_srs: str, out_type: str,
                         nodata: float = -32767.0) -> bool:
    """Write a DGED tile by copying an exactly aligned source window.

    Returns False when the source cannot be proven to be a direct-grid match;
    callers must then use the normal warp path.  This conservative contract is
    important: a near match in mountainous terrain is still a resampling case.
    """
    ...
    if dst_srs:
        try:
            source_srs = osr.SpatialReference(wkt=src.GetProjection())
            target_srs = osr.SpatialReference()
            target_srs.SetFromUserInput(dst_srs)
            try:
                source_srs.StripVertical(); target_srs.StripVertical()
            except AttributeError:
                pass
    ...
    if abs(gt[2]) > 1e-12 or abs(gt[4]) > 1e-12:
        return False
    if abs(abs(gt[1]) - abs(xres)) > max(1e-10, abs(xres) * 1e-8):
        return False
    if abs(abs(gt[5]) - abs(yres)) > max(1e-10, abs(yres) * 1e-8):
        return False
    if src.GetMetadataItem("AREA_OR_POINT") not in ("Point", "point"):
        return False
    ...
    #
    # MEASURED (v0.56.0 review harness, GDAL 3.13.3) -- both outcomes were
    ...
    if abs((gt[0] + (col + 0.5) * gt[1]) - first_x) > 1e-7 or \
       abs((gt[3] + (row + 0.5) * gt[5]) - first_y) > 1e-7:
        return False
    band = src.GetRasterBand(1)
    arr = band.ReadAsArray(col, row, width, height)
    if arr is None:
        return False
    import numpy as np
    arr = np.asarray(arr)
    ...
    if src_nd is not None:
        arr = arr.copy()
        arr[np.isclose(arr.astype("float64"), float(src_nd))] = nodata
    dtype = {"Byte": gdal.GDT_Byte, "UInt16": gdal.GDT_UInt16,
             "Int16": gdal.GDT_Int16, "UInt32": gdal.GDT_UInt32,
             "Int32": gdal.GDT_Int32, "Float32": gdal.GDT_Float32,
             "Float64": gdal.GDT_Float64}.get(out_type, gdal.GDT_Float32)
    drv = gdal.GetDriverByName("GTiff")
    ...
    if dst is None:
        return False
    dst.SetGeoTransform((first_x - xres / 2.0, xres, 0.0,
                         first_y + abs(yres) / 2.0,
                         0.0, -abs(yres)))
    srs = osr.SpatialReference()
    if srs.SetFromUserInput(dst_srs) == 0:
...
def resolve_resampler(input_path: str, src_gsd_m: float, dst_gsd_m: float,
    ...
    source raster (five cheap in-memory warps, no tiles written, no HTML
    report) and returns whichever of Nearest / Bilinear / Cubic / Cubic
    B-Spline / Average reconstructs withheld source posts most accurately
    for THIS input -- cubic-family candidates are scored on the same
    source-range clamp their delivered tiles would actually get, so
    overshoot at a few sharp edges no longer unfairly penalizes them (see
    dem2dged_compare._holdout_stats()'s docstring). It costs extra time
    (roughly one read of the source plus five small warps) that "auto" does
    not, which is why it is opt-in rather than the default.

    v0.57: dst_gsd_m is now passed through to pick_best_resampling() so its
    hold-out test is run at (approximately) the ACTUAL requested decimation
    ratio instead of a hardcoded 2x. Which algorithm reconstructs terrain
    best is ratio-dependent -- a fixed 2x test reliably favours Cubic, but
    a controlled measurement against known ground truth found Nearest
    Neighbor up to 3x more accurate than what the old fixed-2x test would
    have picked, specifically at the large ratios (8x-16x) steep terrain is
    usually delivered at (see dem2dged_v0.56.0_mountain_terrain_review.md).
    Average was also added as a candidate (below) -- it was "auto"'s own
    downsampling default already, and it was the most noise-robust option
    short of Cubic in that same measurement, but it had never actually been
    scored by "optimize" itself.
    ...
    alg, _label, _stats_by_alg = _dc.pick_best_resampling(
        input_path, angular=angular, log_fn=log_fn, dst_gsd_m=dst_gsd_m)
    return alg
